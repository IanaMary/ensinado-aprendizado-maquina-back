...
async def treinar_modelo_generico(
    ...
    **kwargs_adicionais
    ...
    arquivo_doc = await arquivos.find_one({"_id": ObjectId(request.arquivo_id)})
    if not arquivo_doc:
        raise HTTPException(status_code=404, detail="Arquivo não encontrado.")
    
    conf_doc = await configuracoes_treinamento.find_one({"_id": ObjectId(request.configuracao_id)})
    if not conf_doc:
        raise HTTPException(status_code=404, detail="Configuração de treino não encontrada.")
    
    modelo_doc = await opcoes_modelos.find_one({"_id": ObjectId(request.modelo_id)})
    if not modelo_doc:
